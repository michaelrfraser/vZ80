...
public class LoadDEOpCodeTest
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////
	@Test
	public void testLD_DEWord()
	{
		// Set the opcode as the first instruction in memory
		memory.write8( BASE_PC, 0x11 );
		...
		// DE should contain the value of the word param
		Assert.assertEquals( 0xCAFE, this.registers.getDE() );
	}
	
	@Test
	public void testLD_DEWordPointer()
	{
		// Set the opcode as the first instruction in memory
		memory.write8( BASE_PC, 0xED );
		memory.write8( BASE_PC + 1, 0x5B );
		memory.write16( BASE_PC + 2, 0xF00D );
		
		// Execute
		this.vm.tick();
		
		// PC should have been incremented by 4
		Assert.assertEquals( BASE_PC + 4, this.registers.getPc() );
		
		// BC should contain the value that was in the word pointer param
		Assert.assertEquals( 0x1337, this.registers.getDE() );
	}
	//----------------------------------------------------------
	//                     STATIC METHODS
	//----------------------------------------------------------
